...
//    }
	...
	private static CompactMethodNode compactMethodNode(MethodNode mn) {
		...
		cmn.setName(mn.name);
		if (mn instanceof cn.ios.junit.generic.MethodNode) {
			cmn.setMethodSignatureNode(((cn.ios.junit.generic.MethodNode) mn).signatureNode);
		}
		return cmn;
	}
	...
	private static CompactFieldNode compactFieldNode(FieldNode fn) {
		...
		if (fn instanceof cn.ios.junit.generic.FieldNode) {
			cfn.setFieldSignatureNode((((cn.ios.junit.generic.FieldNode) fn).signatureNode));
		}
		return cfn;
	}
